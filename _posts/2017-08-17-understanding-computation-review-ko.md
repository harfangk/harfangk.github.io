...
이뿐 아니라 더 복잡한 개념을 설명할 때도 예시를 보여줍니다. 책의 마지막
부분에서 정지 문제(halting problem)을 다루는데 이 개념도 추상적인 수학적 증명을
...
300쪽을 조금 넘는 책에서 다루기에는 내용이 좀 많아보이는데, 저자가 간결하게
...
수 있더군요. 어째서? 당시에는 어차피 완벽한 것은 있을 수 없다고 대충
넘어갔었는데, 이제는 그런 식으로 오류가 없다는 성질을 보장하는 것 자체가
이론적으로 불가능하다는 것을 이제 알게 되었습니다. 엄격한 타입 시스템으로
특정한 종류의 오류를 박멸할 수는 있어도 모든 오류를 제거하는 것은 불가능합니다.

결국 버그가 없는 소프트웨어를 만들기 위한 그 많은 기술과 기법도 결국 근본적인
이유로 그 목적을 결코 달성할 수는 없다는 말인데, 무모한 일에 도전하는 모습에는
분명 뭔가 비극적인 아름다움이 있긴 하지만 개인적으로는 조금 실망스럽고 안타깝게
느껴지는 결론이었습니다. 예전에 다뤘던 국제안보 분야에 비하면 소프트웨어
개발에는 조금 더 확실한 무엇인가 있기를 바랐는데 말이죠. 원래 그런거다라고
받아들일 수야 있지만, 케이크를 받을 것을 기대하고 있었는데 '케이크는
거짓말이야'라는 말을 들은 느낌입니다. 
...
통해서 조금 더 잘 알게 되었습니다. 튜링 기계, 람다 대수, 룰 110 시스템이나
그 외 기묘한 시스템이 모두 극적으로 다른 형태를 가지면서도 동일한 계산 능력을
가지고 있다는 것을 보고 나니, 널리 쓰이는 프로그래밍 언어들이 동일한 계산
능력을 가지고 있다는 것은 오히려 쉽게 납득할 수 있었습니다. 이는 어떤
프로그래밍 언어가 더 강력한지에 대한 논쟁은 그저 시간 낭비일 뿐이라는 것을
의미합니다.
...
책에서 매우 꼼꼼하게 설명하며, 덕분에 책을 읽었다면 이 코드 내에서 일부 부분을
실제로 인지하고 이해할 수 있습니다. 따라서 이 코드는 오히려 작가가 여기까지
책을 읽고 따라온 독자에게 치는 유쾌한 장난이라고 볼 수 있습니다.
